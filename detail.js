...
const denName = window.location.hash.slice(1);
const denData = predpoved.find((denVTydnu) => denVTydnu.datum === denName);
...
mainElement.innerHTML += `
  <article>
    <h2>${denData.den}</h2>
    <p>${denData.datum}</p>
    <p>${denData.ranni_teplota}</p>
    <p>${denData.odpoledni_teplota}</p>
    <p>${denData.vecerni_teplota}</p>
    <p>${denData.stav_pocasi}</p> 
    <p>${denData.tlak}</p>
    <p>${denData.rychlost_vetru}</p>
